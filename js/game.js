...
GC.game = {
    ...
    draw_frame: function (ts) {
        ...
        if (this.animation_previous_ts != ts) {
            ...
            let nt_display_x = next_tile_x + Math.floor((GC.overworld.columns - GC.options.tile_columns) / 2) - Math.round(GC.display.display_offset.x / GC.options.tile_size);
            let nt_display_y = next_tile_y + Math.floor((GC.overworld.rows - GC.options.tile_rows) / 2) - Math.round(GC.display.display_offset.y / GC.options.tile_size);

            if (nt_display_x < 0) {
                nt_display_x += GC.overworld.columns;
            } else if (nt_display_x > GC.overworld.columns - 1) {
                nt_display_x -= GC.overworld.columns;
            }

            if (nt_display_y < 0) {
                nt_display_y += GC.overworld.rows;
            } else if (nt_display_y > GC.overworld.rows - 1) {
                nt_display_y -= GC.overworld.rows;
            }
            ...
            if (!this.block_ahead) {
                ...
                GC.display.display_offset.x += partial_offset * this.offset_vectors[GC.player.current_direction].x;
                GC.display.display_offset.y += partial_offset * this.offset_vectors[GC.player.current_direction].y;

                if (offset >= GC.options.tile_size) {
                    GC.display.display_offset.x = GC.tools.nearestMultiple(GC.display.display_offset.x, GC.options.tile_size);
                    GC.display.display_offset.y = GC.tools.nearestMultiple(GC.display.display_offset.y, GC.options.tile_size);
                }
...
GC.player = {
    ...
    sprite_map:{
        ...
        ArrowUp_alt: [128, 160],
        ArrowLeft: [192, 224],
        ArrowLeft_alt: [192, 224],
        ArrowRight: [256, 288],
        ArrowRight_alt: [256, 288]
    },
    ...
    init:function(){
        ...
        this.sprite_src.addEventListener("load",function(){this.draw()}.bind(this));
        this.draw();
