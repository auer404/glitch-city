GC.overworld = {
    ...
    let gc_tile = GC.overworld.tile(x, y);

    if (gc_tile.uses) { return; }

    gc_tile.uses = GC.tileset_map_get(tile_id, tolerance);
    ...
    if (propagate) {
        setTimeout(function () {
            for (n of gc_tile.neighbors) {
                GC.overworld.set_pattern(n.x, n.y, true, gc_tile.uses.id, tolerance);
            }
        }.bind(this), 1);
    }
    ...
    if (this.tiles_set >= this.tiles.length) {
        GC.display.draw();
        GC.events.trigger("overworldReady");
    }
...
init:function() {
    ...
    GC.preloader.register(this.tiles.length, true);
